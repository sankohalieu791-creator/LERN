...
-- CREATE OR REPLACE VIEW can only APPEND columns, never insert one
-- into the middle of the existing list (Postgres treats that as
-- renaming whatever column now sits at that position) -- video_path
-- goes at the very end, keeping every original column's position
-- exactly as it was.
