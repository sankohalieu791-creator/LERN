...
function EmployerThread({ item, onBack }: { item: any; onBack: () => void }) {
  ...
  return (
    // Same messaging-platform shape as InterestReceivedPanel's own
    // thread view: a bounded-height flex column (header, flex-1
    // scrolling messages, composer as the last child) so the composer
    // sits at the true bottom of the screen regardless of how short
    // the conversation is -- fixed full-screen on phone, the plain
    // in-page card on desktop.
    <div className="fixed inset-0 z-40 flex flex-col bg-paper lg:static lg:z-auto lg:flex lg:flex-col lg:bg-transparent">
      <div className="flex-shrink-0 flex items-center gap-3 bg-paper border-b border-edge px-4 py-3 lg:hidden" style={{ paddingTop: 'calc(env(safe-area-inset-top) + 0.75rem)' }}>
        ...
        <div className="min-w-0">
          <p className="font-bold text-ink text-[14px] truncate">{firstName}{item.opportunity_label ? ` · ${item.opportunity_label}` : ''}</p>
          <p className="text-[12px] text-ink-tertiary truncate">Routed through their organisation</p>
        </div>
        <span className="flex-shrink-0 ml-auto text-[11px] font-semibold px-2.5 py-1 rounded-full" style={{ backgroundColor: status.bg, color: status.fg }}>{status.label}</span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 pt-4 lg:flex-none lg:overflow-visible lg:px-0 lg:pt-0">
        ...
        <div className="bg-transparent lg:bg-surface border-0 lg:border lg:border-edge rounded-xl lg:p-5 mb-4">
          <div className="hidden lg:flex items-center justify-between gap-2 mb-1">
      ...
      <p className="hidden lg:flex items-center gap-1.5 text-[12px] text-ink-quaternary flex-shrink-0 mt-2">
