    ...
    <div className="max-w-xl mx-auto space-y-4">
      ...
      {loading ? (
        <div className="space-y-3">
          {[0, 1, 2].map(i => <div key={i} className="h-32 rounded-2xl bg-[#F5F1E8] animate-pulse" />)}
        </div>
      ) : posts.length === 0 ? (
...
function Composer({ onPosted }: { onPosted: () => void }) {
  ...
  return (
    ...
    <div className="lg:hidden bg-white border border-[#E2DDD1] rounded-2xl p-4">
      <div className="flex items-start gap-2.5">
        <div className="w-9 h-9 rounded-full bg-[#F5F1E8] flex items-center justify-center text-[12px] font-bold text-[#6B6558] flex-shrink-0">
        ...
        <textarea
          ...
          rows={2}
          className="flex-1 text-[14px] text-ink placeholder-[#A39C8A] outline-none resize-none"
        />
      </div>
      {preview && (
        <div className="relative mt-2 ml-11">
          <img src={preview} alt="" className="max-h-48 rounded-xl border border-[#E2DDD1]" />
      ...
      <div className="flex items-center justify-between mt-3 ml-11">
        <div className="flex items-center gap-1">
          <button onClick={() => fileRef.current?.click()} className="w-8 h-8 rounded-lg flex items-center justify-center text-[#6B6558] hover:bg-[#F5F1E8] transition">
          ...
          {isAdult && (
            <button
              ...
              className="flex items-center gap-1 text-[11px] font-semibold text-[#6B6558] px-2 py-1 rounded-lg hover:bg-[#F5F1E8] transition"
            >
      ...
      <div className="flex items-center justify-between mb-2.5">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[11px] text-[#8A8373] flex items-center gap-1">
              {timeAgo(post.created_at)} · {post.visibility === 'public' ? <Globe className="w-3 h-3" /> : <Users className="w-3 h-3" />}
            </p>
          </div>
        </div>
        {canDelete && (
          <button onClick={remove} className="w-7 h-7 rounded-lg flex items-center justify-center text-[#A39C8A] hover:text-[#B3401E] hover:bg-[#FDEEEA] transition">
            <Trash2 className="w-3.5 h-3.5" />
          </button>
        )}
      </div>

      {post.content && <p className="text-[14px] text-[#2A261F] whitespace-pre-wrap mb-2.5 leading-relaxed">{post.content}</p>}
      {imageUrl && <img src={imageUrl} alt="" className="w-full rounded-xl border border-[#EDE9E1] mb-2.5" />}

      {counts.length > 0 && (
        <div className="flex items-center gap-1 mb-2 text-[12px] text-[#8A8373]">
          {counts.map(c => <span key={c.key}>{c.emoji}</span>)}
          <span className="ml-1">{reactions.length}</span>
        </div>
      )}

      <div className="flex items-center gap-1 pt-2.5 border-t border-[#EDE9E1]">
        {REACTIONS.map(r => (
          <button
            ...
            className={`flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[12px] font-semibold transition ${
              myReaction === r.key ? 'bg-[#FCEEE4] text-brand' : 'text-[#6B6558] hover:bg-[#F5F1E8]'
            }`}
          >
            <span>{r.emoji}</span>
