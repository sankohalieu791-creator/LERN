...
export default function StudentShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div data-theme="dark" className="min-h-screen bg-[#0f0f0f] flex flex-col" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
      ...
      <main className="flex-1 overflow-y-auto" style={{ paddingBottom: 'calc(88px + env(safe-area-inset-bottom))' }}>{children}</main>

      <nav
        className="fixed bottom-0 left-0 right-0 bg-[#0f0f0f] border-t border-white/10 flex items-center justify-around h-[88px] z-30"
...
function NavItem({ href, icon: Icon, label, active }: { href: string; icon: any; label: string; active: boolean }) {
  return (
    <Link href={href} className={`flex flex-col items-center justify-center gap-1 flex-1 h-full ${active ? 'text-white' : 'text-[#666]'}`}>
      <Icon className="w-8 h-8" strokeWidth={active ? 2.4 : 2} />
...
function PlusButton() {
  return (
    <div aria-disabled className="flex items-center justify-center flex-1 h-full text-[#555] cursor-not-allowed">
      <Plus className="w-10 h-10 transition-transform duration-150 active:rotate-45" strokeWidth={2.2} />
    </div>
  )
}
...
function DisabledNavItem({ icon: Icon, label }: { icon: any; label: string }) {
  return (
    <div aria-disabled className="flex flex-col items-center justify-center gap-1 flex-1 h-full text-[#3a3a3a] cursor-not-allowed">
